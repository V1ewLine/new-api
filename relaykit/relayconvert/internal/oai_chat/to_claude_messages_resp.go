...
func stopOpenBlocks(state *convmeta.ClaudeConvertInfo) []*dto.ClaudeResponse {
	...
	case convmeta.LastMessageTypeTools:
		blockIndexes := make([]int, 0, len(state.ToolCallBlockIndexes))
		for _, blockIndex := range state.ToolCallBlockIndexes {
...
func appendClaudeToolCallDeltas(
	...
	state *convmeta.ClaudeConvertInfo,
) []*dto.ClaudeResponse {
	if state.ToolCallBlockIndexes == nil {
		state.ToolCallBlockIndexes = make(map[int]int)
	}
	if state.PendingToolCalls == nil {
		state.PendingToolCalls = make(map[int]dto.ToolCallResponse)
	}
	...
	for i, toolCall := range toolCalls {
		...
		blockIndex, started := state.ToolCallBlockIndexes[sourceIndex]
		if !started {
			if pending, ok := state.PendingToolCalls[sourceIndex]; ok {
				if toolCall.ID == "" {
			...
			if toolCall.Function.Name == "" {
				state.PendingToolCalls[sourceIndex] = toolCall
				continue
			}
			...
			state.ToolCallBlockIndexes[sourceIndex] = blockIndex
			state.Index = blockIndex
			delete(state.PendingToolCalls, sourceIndex)
...
func StreamResponseOpenAI2Claude(openAIResponse *dto.ChatCompletionsStreamResponse, info convmeta.Meta) []*dto.ClaudeResponse {
	...
	// For tools, OpenAI tool_calls can stream multiple parallel tool_use blocks,
	...
	stopOpenBlocksAndAdvance := func() {
		...
		case convmeta.LastMessageTypeTools:
			...
			state.ToolCallBlockIndexes = nil
	...
	if info.GetSendResponseCount() == 1 {
		...
		if openAIResponse.IsToolCall() {
			...
			claudeResponses = appendClaudeToolCallDeltas(claudeResponses, toolCalls, state)
		}
